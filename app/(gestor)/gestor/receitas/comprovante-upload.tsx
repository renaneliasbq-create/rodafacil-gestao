...
import { salvarComprovante, deletarComprovante } from './actions'
...
export function ComprovanteUpload({ pagamentoId, comprovanteUrl, comprovantePath }: Props) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 10 * 1024 * 1024) {
      setErro('Arquivo deve ter no máximo 10 MB.')
      return
    }
    setErro(null)
    startTransition(async () => {
      ...
      const path = `${pagamentoId}/${Date.now()}.${ext}`
      ...
      const result = await salvarComprovante(pagamentoId, data.publicUrl, path)
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      await deletarComprovante(pagamentoId, comprovantePath ?? '')
      setUrl(null)
    })
  }
