...
export function FiltroPeriodo() {
  ...
  function navTo(search: string) {
    router.push('/gestor/relatorios?' + search)
  }
      ...
      {p === 'custom' && (
        <div className="flex items-center gap-2 mt-1 w-full sm:w-auto sm:mt-0">
          ...
          <button
            ...
            disabled={!ini || !fim}
            className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-blue-600 text-white disabled:opacity-40 hover:bg-blue-700 transition-colors"
          >
            Filtrar
          </button>
        </div>
